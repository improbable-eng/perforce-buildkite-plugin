...
# The base revision to sync to in this build
__REVISION_METADATA__ = 'buildkite:perforce:revision' 
__REVISION_ANNOTATION__ = "Revision: %s"

# CL number of a shelved changelist to unshelve into the workspace for each job
__SHELVED_METADATA__ = 'buildkite:perforce:shelved'
__SHELVED_ANNOTATION__ = "[{timestamp}] Saved shelved change {original} as {copy}"

# Revision identifier that should be unique for base revision + shelved change combos
__COMMIT_METADATA__ = 'buildkite:git:commit' 
...
def set_build_commit(revision, changelist):
    commit = '+'.join(filter(None, [revision, changelist])) # revision or revision+changelist
    set_metadata(__COMMIT_METADATA__, commit)
